...
| 变量 | 必需 | 说明 |
| --- | --- | --- |
| `GEMINI_API_KEY` | AI 功能必需 | 服务端 Gemini API Key |
...
`GEMINI_API_KEY` 只由 [`/api/ai-score`](src/app/api/ai-score/route.ts) 在服务端读取，
并通过 `x-goog-api-key` 请求头发送，不会打包进浏览器。请勿给密钥添加
`NEXT_PUBLIC_` 前缀，也不要提交 `.env.local` 或 `.env.production`。
